...
## 0. Data
#Same as MapUCEtoGenome_general.sh

######################################################################

## 1. Extract UCE loci from genome and treat as contigs
#Convert .fna to .2bit
qsub -I -l nodes=1:ppn=12 -l mem=100gb -l walltime=8:00:00
cd /home/mapUCE
...
#Create config file
printf [scaffolds]'\n' > genomes.conf
ls -1d genome-lastz/ | sed 's/\.lastz\.clean//; s/Cowman_etal_APPENDIX_C-hexa-v2-final-probes\.fasta_v_//; h; 1! s/$/:\/home\/jc313394\/genomes\/archives\//; 1! G; 1! s/\n//; 1! G; s/\n/\//; 1! s/$/\.2bit/' >> genomes.conf
...
singularity run /fast/tmp/containers/phyluce-1.7.1.sif phyluce_probe_slice_sequence_from_genomes \
    ...
    --output genome-contigs \
    --log-path log
#Keep the log to compare it with the mapping process
grep "written" log/phyluce_probe_slice_sequence_from_genomes.log | sed 's/.*INFO\ -\ //; s/, /,/g; s/\: /,/' > results_probeslice.txt

#Rename files because everything gets lowercased :(
cd genome-contigs
for fa in *; do   
    sample=$(echo $fa | sed '1 s/^a/A/; s/gca/GCA/; s/gcf/GCF/; s/reefgenomics/ReefGenomics/; s/cyanophora/Cyanophora/')
...
cd /home/mapUCE/tree
mkdir -p {3_correctedcontigs,4_match/log,5_align/log,6_tree}
...
singularity run /fast/tmp/containers/phyluce-1.7.1.sif phyluce_align_seqcap_align \
    --input ../4_match/taxon-map/taxa-incomplete.fasta \
    ...
    --log-path log
#trimAL trimming
singularity run /sw/containers/phyluce-1.7.2.sif phyluce_align_get_trimal_trimmed_alignments_from_untrimmed \
    --alignments mafft-nexus-internal-trimmed \
    --output mafft-nexus-internal-trimmed-trimal \
    --cores 12 \
    --log log
#Summary stats
singularity run /sw/containers/phyluce-1.7.2.sif phyluce_align_get_align_summary_data \
    --alignments mafft-nexus-internal-trimmed-trimal \
    --cores 12 \
    --log-path log 
#Remove locus names
singularity run /sw/containers/phyluce-1.7.2.sif phyluce_align_remove_locus_name_from_files \
    --alignments mafft-nexus-internal-trimmed-trimal \
    --output mafft-nexus-internal-trimmed-trimal-clean \
    --cores 12 \
    --log-path log
#Get % complete matrix (change percent value and output name)
singularity run /sw/containers/phyluce-1.7.2.sif phyluce_align_get_only_loci_with_min_taxa \
...
singularity run /sw/containers/phyluce-1.7.2.sif phyluce_align_get_informative_sites \
...
singularity run /sw/containers/iqtree-2.2.2.2.sif iqtree2 \
    ...
    -T AUTO --threads-max 20 -m MFP+MERGE -rcluster 10 -alninfo --cptime 300
#Infer gene/locus trees
singularity run /sw/containers/iqtree-2.2.2.2.sif iqtree2 \
    -S $alignment --prefix loci.$run -T AUTO --threads-max 10 --cptime 300
#Compute gCF
singularity run /sw/containers/iqtree-2.2.2.2.sif iqtree2 \
...
singularity run /sw/containers/iqtree-2.2.2.2.sif iqtree2 \
    ...
    --prefix siteconcord.$run -T AUTO --threads-max 20 --cf-verbose --cf-quartet
